import psycopg2
import re
#yearmonthday - 19970130
def selectFighterNames(dates):
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT fighterName FROM fighters WHERE FightTime BETWEEN " + "to_date('"+dates[0]+"','YYYYMMDD')" + " AND " + "to_date('"+dates[1]+"','YYYYMMDD');")
...
if __name__ == "__main__":
        ...
        f_names = selectFighterNames(("20170615","20170716"))
        for name in f_names:
            ima = re.sub(r'-','',str(name[0]))
            if(tableExist(str(ima)): 
